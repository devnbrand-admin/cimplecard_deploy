    ...
    <div className="w-full bg-[#121F2E] text-white py-16 mt-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 px-6 md:px-12 lg:px-24 font-Figtree">
          {/* YouTube Video Cards */}
          {
            card?.youtubeVideoLink?.map((video, index) => (
              <div key={index} className="bg-transparent rounded-lg shadow-lg overflow-hidden">
                <iframe
                  src={video}
                  width="100%"
                  height="400"
      ...
      <div>
        ...
        >
          ...
          {
            card?.instagramPost?.map((post, index) => (
              <div key={index} className="flex basis-[150px] grow flex-1 flex-col items-center justify-center px-6 md:px-12 lg:px-24">
                <iframe
                  src={`${post}/embed`}
                  width="400"
                  height="600"
