...
export const updateCard = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, jobTitle, companyName, services, SocialMediaLink, testimonials, businessHours, companySocialMediaLink, ...otherCardDetails } = req.body;
        const userId = req.user?.id;
        ...
        if (companySocialMediaLink) {
            await Promise.all(companySocialMediaLink.map(async (link) => {
                if (link.id) {
        ...
        if (SocialMediaLink) {
            await Promise.all(SocialMediaLink.map(async (link) => {
                if (link.id) {
